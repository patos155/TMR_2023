...
void mostrar_valores (int16_t *gyro, int16_t *accel, int32_t *quat, uint32_t *timestamp) {	
  ...
  spamtimer(SpamDelay) {			// si han transcurrido al menos 100 mseg entonces proceder
    ...
    /*Serial.printfloatx(F("")  , xyz[0], 0, 4, F(","));  // muestra en monitor serie rotacion de eje Z, yaw
    Serial.printfloatx(F(""), xyz[1], 0, 4, F(","));  // muestra en monitor serie rotacion de eje Y, pitch
    Serial.printfloatx(F("") , xyz[2], 0, 4, F(""));  // muestra en monitor serie rotacion de eje X, roll*/
    Serial.print(xyz[0]); //z yaw
    Serial.print(",");
    Serial.print(xyz[1]); //y pitch
    Serial.print(",");
    Serial.print(xyz[2]); //x roll
